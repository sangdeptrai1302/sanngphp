      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6 ">
          <div class="row float-right">
            ...
            <a class="btn btn-danger mr-1 px-1" href="index.php?option=post&cat=trash"><i class="fa-solid fa-trash"></i>Thùng Rác</a>
          </div>
        </div>
      </div>
    </div><!-- /.container-fluid -->
  ...
  <section class="content">
    ...
    <script>
      $(document).ready(function() {
        ...
        $('.toggle-btn').click(function(e) {
          e.preventDefault();
          var id = $(this).data('id'); // lấy id của post
    ...
    <div class="card">
      <div class="card-header">
        <form method="post" action="index.php?option=post&cat=delete">
        ...
        <table class="table table-bordered table-hover table-striped">
          <thead>
            <tr>
              ...
              <th style='width:200px;'>chi tiết bài viết</th>
              <th style='width:200px;'>từ khóa</th>
              <th style='width:200px;'>Từ Khóa Mô Tả SEO</th>
              <th>Ngày Tạo</th>
              <th style='width:450px;' >Chức năng</th>
            </tr>
          </thead>
          <tbody>
            <?php foreach ($list as $row) : ?>
              <tr>
                ...
                <td><?= $row['type'] ?></td>
                <td><?= $row['slug'] ?></td>
                <td><?= substr($row['detail'],50,100) ?></td>
                <td><?= $row['metakey'] ?></td>
                <td><?= $row['metadesc'] ?></td>
                ...
                <td>
                  <div class="container " style="align-items:center">
                    <?php if ($row['status'] == 1) : ?>
                      <button class="btn btn-sm btn-success p-3 m-2 toggle-btn" name="id" data-id="<?= $row['id']; ?>" style="border-radius: 17%" href="index.php?option=post&cat=status&id=<?= $row['id']; ?>">
                        <i class="fas fa-toggle-on"></i>
                      </button>
                    <?php else : ?>
                      <button class="btn btn-sm btn-danger p-3 m-2 toggle-btn" name="id" data-id="<?= $row['id']; ?>" style="border-radius: 17%" href="index.php?option=post&cat=status&id=<?= $row['id']; ?>">
                        <i class="fas fa-toggle-off"></i>
                      </button>
                    <?php endif; ?>
                    <a class="btn btn-sm btn-info p-3 m-2" style="border-radius: 17%" ; href="index.php?option=post&cat=detail&id=<?= $row['id']; ?>">
                      <i class="fa-solid fa-circle-info"></i>
                    </a>
                    <a class="btn btn-sm btn-success p-3 m-2" style="border-radius: 17%" ; href="index.php?option=post&cat=edit&id=<?= $row['id']; ?>">
                      <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a class="btn btn-sm btn-danger  p-3 m-2" style="border-radius: 17%" ; href="index.php?option=post&cat=delete&id=<?= $row['id']; ?>">
